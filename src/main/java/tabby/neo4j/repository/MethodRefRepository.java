...
public interface MethodRefRepository extends Neo4jRepository<MethodReference, UUID> {
    ...
    @Query("CALL apoc.periodic.iterate(\"CALL apoc.load.csv('file://\"+$path+\"', {header:true, mapping:{ isStatic: {type:'boolean'}, hasParameters:{type:'boolean'}, isSink: { type: 'boolean'}, isSource: { type: 'boolean'}, isPolluted: { type: 'boolean'}, parameters:{array:true, arraySep:'|'}, pollutedPosition:{array:true, arraySep:'|'} }}) YIELD map AS row RETURN row\", \"MERGE(m:Method {uuid:row.uuid} ) ON CREATE SET m = row\", {batchSize:5000, iterateList:true, parallel:true})")
    void loadMethodRefFromCSV(String path);

    @Query("CALL apoc.periodic.iterate(\"CALL apoc.load.csv('file://\"+$path+\"', {header:true, mapping:{ isPolluted: {type: 'boolean'}, pollutedPosition: {array:true, arraySep:'|'} }}) YIELD map AS row RETURN row\",\"MATCH ( m1:Method {uuid:row.source} ) MATCH ( m2:Method {uuid:row.target }) MERGE (m1)-[e:CALL {uuid:row.uuid, lineNum:row.lineNum, isPolluted:row.isPolluted, pollutedPosition:row.pollutedPosition, realCallType:row.realCallType, invokerType: row.invokerType }]->(m2)\", {batchSize:5000, iterateList:true, parallel:false})")
